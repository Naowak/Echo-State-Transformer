...
class GRU(nn.Module):
    def __init__(self, hidden_size=10, num_layers=1, device='cpu'):
        """
        Classe pour un modèle GRU.

        Paramètres :
        - hidden_size (int) : Dimension de l'espace caché.
        - num_layers (int) : Nombre de couches GRU.
    ...
    def run_training(self, X_train, Y_train, T_train, X_valid=None, Y_valid=None, T_valid=None, epochs=100, batch_size=32, 
        ...
        for epoch in range(epochs):
            ...
            for i, (batch_X, batch_Y) in enumerate(dataloader):
                ...
                # Forward pass
                outputs, _ = self.model(batch_X, h0) # outputs: (batch_size, seq_length, hidden_size)
    ...
    def run_inference(self, X, batch_size=32):
        """
        Génère des prédictions avec le modèle GRU.
        ...
        with torch.no_grad():
            for i in range(0, X.shape[0], batch_size):
                ...
                # Forward pass
                batch_outputs, _ = self.model(X_batch, h0)
    ...
    def _define_model(self, input_size, output_size, learning_rate, weight_decay, classification):
        ...
        self.model = nn.GRU(input_size, self.hidden_size, self.num_layers, batch_first=True, device=self.device)
